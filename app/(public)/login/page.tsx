...
export default function LoginPage() {
  ...
  const submit = async (e: SubmitEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error("Error", {
          description: data.error ?? "Unable to log in.",
        });
        return;
      }
      // Cookie is set server-side (httpOnly) — just sync the UI state
      setUser(data.user);
      router.push("/dashboard");
    } catch (err) {
      console.error("Login error:", err);
      toast.error("Error", { description: "Unable to log in." });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden transition-colors duration-500">
      ...
      <Magnet padding={85} magnetStrength={50}>
        <div className="relative z-10 w-full max-w-md rounded-2xl bg-white dark:bg-black/30 backdrop-blur-xl p-8 shadow-2xl transition-colors duration-500">
          ...
          <p className="text-center text-gray-700 dark:text-white/70 text-sm mt-3">
            <Link
